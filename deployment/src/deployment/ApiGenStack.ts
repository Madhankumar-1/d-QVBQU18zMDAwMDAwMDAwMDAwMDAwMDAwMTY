...
import {handlersPath} from "@cny-cdk-stack/test-goal-stack-lambda";
...
export class ApiGenStack extends AwsServerlessStackBase {
     ...
     async doDeployment(): Promise<void> {
          ...
          const goalsTable = await this.createDynamodbTable(`goal-test`, {
          ...
          if (this.isAuthorizationExists) {
               // Authorizer
               this.authorizerLambdaPath = `${handlersPath}/authorizer/src/index.js`;
          ...
          this.endpoints.forEach(async (endpoint) => {
               const environment = {
                    STAGE: this.stage!,
                    DEFAULT_DYNAMODB_TABLE_NAME: goalsTable.tableName,
               };

               const lambdaPath = `${handlersPath}/${endpoint.serviceMethodName}/src/index.js`;
